// module/turn-cards.ts
// Turn Cards HUD - Combat UI for Masks: A New Generation
...
// Uses masks module namespace for storage (shared resource)
// ────────────────────────────────────────────────────────────────────────────
...
export async function promptShiftLabels(actor, title) {
...
export async function applyShiftLabels(actor, upKey, downKey, { announce = true, reason = "shift", sourceActor = null } = {}) {
	const { lo, hi } = shiftBounds();
...
// ────────────────────────────────────────────────────────────────────────────
// Query Handlers (v13 Query System)
// ────────────────────────────────────────────────────────────────────────────

function registerQueryHandlers() {
	// Mark Action
	CONFIG.queries[`${DISPATCH_NS}.markAction`] = async function (data) {
	...
	// Aid Teammate
	CONFIG.queries[`${DISPATCH_NS}.aidTeammate`] = async function (data) {
...
${AID_MOVE_UUID}`;
	...
	// Change Potential
	CONFIG.queries[`${DISPATCH_NS}.changePotential`] = async function (data) {
	...
	// Change Forward
	CONFIG.queries[`${DISPATCH_NS}.changeForward`] = async function (data) {
		const { actorId, delta, userId } = data;
		const actor = game.actors?.get?.(actorId);
		const user = game.users?.get?.(userId);
	...
	// Change Team Pool
	CONFIG.queries[`${DISPATCH_NS}.changeTeam`] = async function (data) {
		const { delta } = data;
	...
	// Shift Labels
	CONFIG.queries[`${DISPATCH_NS}.shiftLabels`] = async function (data) {
		const { targetActorId, up, down, reason } = data;
		const target = game.actors?.get?.(targetActorId);
		if (!target || !up || !down) {
	...
	console.log(`[${DISPATCH_NS}] Query handlers registered`);
}
...
function registerSocketHandler() {
	if (!game.socket) {
		console.warn(`[${DISPATCH_NS}] Socket not available`);
		return;
	}
	...
	console.log(`[${DISPATCH_NS}] Socket handler registered`);
}
...
async function queryGM(queryName: string, data: object, options: { timeout?: number } = {}) {
	...
	const promise = (async () => {
		try {
			const result = await gm.query(`${DISPATCH_NS}.${queryName}`, data, { timeout: options.timeout ?? 10000 });
			if (result && typeof result === 'object' && 'success' in result && !result.success && 'error' in result) {
				ui.notifications?.warn?.(String(result.error));
			}
			return result;
		} catch (err) {
			console.error(`[${DISPATCH_NS}] Query failed:`, err);
...
const TurnCardsHUD = {
	...
	mount() {
		...
		this.root.id = "dispatch-turncards";
	...
	_registerHooks() {
		...
		this._hooks.on("canvasReady", () => {
			if (!document.getElementById("dispatch-turncards")) this.mount();
...
export function registerTurnCards() {
	registerQueryHandlers();
	registerSocketHandler();
	TurnCardsHUD.mount();
}

export function cleanupTurnCards() {
	TurnCardsHUD.unmount();
}
